...
/**
 * Ein Behaviour, das auf dem Client läuft, also hauptsächlich optische Aufgaben hat
 *
 * ID-Liste:
 *
 ...
 * 7 - Harvest - Ernten
 * 8 - Upgrade - Wartezeit bis das Upgrade fertig ist managen
 * 9 - Produce - Rohstoffe produzieren (nicht ernten) (nur Gebäude)
 * @author tfg
 */
public abstract class ClientBehaviour implements Pauseable {

    protected int id;       // Nummer (Art) des Behaviours. Viel schneller als die alten Strings.
    ...
    public ClientCore.InnerClient rgi; // Referenz auf alles, damit können Behaviours auch alles machen
     ...
     * @param newinner
     */
    public ClientBehaviour(ClientCore.InnerClient newinner, GameObject caster, int ID, int callsPerSecond, boolean createAsActive) {
        rgi = newinner;
        id = ID;
     ...
     * @param newinner
     */
    public ClientBehaviour(ClientCore.InnerClient newinner, GameObject caster, int ID, double callsPerSecond, boolean createAsActive) {
        rgi = newinner;
        id = ID;
