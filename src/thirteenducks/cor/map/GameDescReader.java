...
public class GameDescReader {
	...
	for (File descFile : alist) {
		...
		bdescReader = new BufferedReader(descReader);
		String zeile = null;
		// Einlesen
		...
		while ((zeile = bdescReader.readLine()) != null) {
			...
			if (inDesc) {
				...
				if (type == 0) {
					...
					if (v2.contains("all")) {
					    au.affects = thirteenducks.cor.game.ability.AbilityUpgrade.upgradeaffects.all;
					} else if (v2.contains("fresh")) {
					    au.affects = thirteenducks.cor.game.ability.AbilityUpgrade.upgradeaffects.fresh;
					} else if (v2.contains("old")) {
					    au.affects = thirteenducks.cor.game.ability.AbilityUpgrade.upgradeaffects.old;
					} else if (v2.contains("self")) {
					    au.affects = thirteenducks.cor.game.ability.AbilityUpgrade.upgradeaffects.self;
					    au.useForAll = true;
					}
	...
	for (File descFile : alist) {
		...
		bdescReader = new BufferedReader(abReader);
		String zeile = null;
		// Einlesen
		boolean inDesc = false;
		...
		while ((zeile = bdescReader.readLine()) != null) {
			...
			if (inDesc) {
				...
				if (type == 0) {
					...
					if (v2.contains("all")) {
					    au.affects = thirteenducks.cor.game.ability.ServerAbilityUpgrade.upgradeaffects.all;
					} else if (v2.contains("fresh")) {
					    au.affects = thirteenducks.cor.game.ability.ServerAbilityUpgrade.upgradeaffects.fresh;
					} else if (v2.contains("old")) {
					    au.affects = thirteenducks.cor.game.ability.ServerAbilityUpgrade.upgradeaffects.old;
					} else if (v2.contains("self")) {
					    au.affects = thirteenducks.cor.game.ability.ServerAbilityUpgrade.upgradeaffects.self;
					}
				    } else if ("uptype".equals(v1)) {
					if (v2.equals("epoche")) {
		...
		serverrgi.logger(ex);
		serverrgi.logger("[ServerMapModule][ERROR]: Can't load abilit-list " + descFile.getAbsolutePath());
	    } catch (IOException ex) {
		serverrgi.logger(ex);
		serverrgi.logger("[MapModule][ERROR]: Can't read abilit-list " + descFile.getAbsolutePath());
	    } finally {
		try {
		    abReader.close();
		} catch (IOException ex) {
