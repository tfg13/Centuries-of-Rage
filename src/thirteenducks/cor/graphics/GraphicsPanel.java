...
public class GraphicsPanel extends javax.swing.JLabel {
            ...
            if (hoverx != -1) {
                ...
                if (countImage >= 0 && countImage < images.size()) {
                    ...
                    if (descModi == 'b') {
                        ...
                        String tempStr = null;
                        tempStr = descBuildings.get(countImage).getName();
                        if (tempStr != null) {
                            g.drawString(tempStr, 2, renderY + 10);
                        }
                        tempStr = descBuildings.get(countImage).getZ1() + " x " + descBuildings.get(countImage).getZ2();
                        // Größe
                        if (tempStr != null) {
                            g.drawString(tempStr, getWidth() - 25, renderY + 10);
                        }
                        tempStr = descBuildings.get(countImage).getDescDescription();
                        if (tempStr != null) {
                            // Beschreibung
                        ...
                        g.setFont(Font.decode("Arial-10"));
                        String tempStr = null;
                        tempStr = descUnits.get(countImage).getName();
                        if (tempStr != null) {
                            g.drawString(tempStr, 2, renderY + 10);
                        }
                        // Beschreibung
                        tempStr = descUnits.get(countImage).getDescDescription();
                        if (tempStr != null) {
                            // Beschreibung
                            g.drawString(tempStr, 2, renderY + 19);
                        }
                        ...
                        if (countImage == 0) {
                            ...
                            tempStr = "+ " + descUnits.get(countImage).getDescPro() + " | - " + descUnits.get(countImage).getDescCon();
                        }
                        if (tempStr != null) {
    ...
    public int getSelectedDesc() {
        if (descModi == 'b') {
            return descBuildings.get(selectedIndex).getDescTypeId();
        } else {
            return descUnits.get(selectedIndex).getDescTypeId();
        }
    }
